...
export default function Input({
	...
	const changeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
		groupValue ? setValue((prev: any) => ({ ...prev, [name]: e.target.value })) : setValue(e.target.value);
		!!onChange && onChange(e);
	};
			...
			<div className="relative w-full">
				...
				{!!placeholder && (
					<span
						...
						className={`whitespace-nowrap absolute duration-200 z-10 bg-white px-2  ${
							focused ? "left-3 -top-2 text-xs " : "left-3 top-3"
						}`}
					>
						{placeholder}
					</span>
				...
				<span
					onClick={handleClear}
					className={`whitespace-nowrap absolute duration-200 z-10 bg-white px-2 right-3 top-4 cu  ${
